...
export const AdminReports = () => {
  ...
  const handleExport = (format: 'json' | 'csv') => {
    ...
    if (format === 'json') {
      ...
      a.href = url;
      a.download = `greenchain-report-${new Date().toISOString().split('T')[0]}.json`;
      a.click();
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `greenchain-report-${new Date().toISOString().split('T')[0]}.csv`;
      a.click();
      URL.revokeObjectURL(url);
    }
  };
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-400">Comprehensive system insights and data exports</p>
          </div>
          ...
          <Card className="bg-gradient-to-br from-emerald-500/20 to-emerald-600/20 border-emerald-500/30">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-slate-400 text-sm mt-2">System-wide</p>
              </div>
          ...
          <Card className="bg-gradient-to-br from-cyan-500/20 to-cyan-600/20 border-cyan-500/30">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-slate-400 text-sm mt-2">In circulation</p>
              </div>
          ...
          <Card className="bg-gradient-to-br from-blue-500/20 to-blue-600/20 border-blue-500/30">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-slate-400 text-sm mt-2">Issued to date</p>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card>
            <h3 className="text-xl font-bold text-white mb-4">Weekly Energy Trend</h3>
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={weeklyTrend}>
                ...
                />
                <Legend />
                <Line type="monotone" dataKey="generation" stroke="#10b981" strokeWidth={3} name="Generation" />
                <Line type="monotone" dataKey="consumption" stroke="#3b82f6" strokeWidth={3} name="Consumption" />
              </LineChart>
            </ResponsiveContainer>
          </Card>
        </div>
        ...
        <Card>
          <h3 className="text-xl font-bold text-white mb-4">Monthly Activity Trends</h3>
        ...
        <Card>
          <h3 className="text-xl font-bold text-white mb-4">Generator Performance Comparison</h3>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={generatorComparison}>
              ...
              />
              <Legend />
              <Bar dataKey="generated" fill="#10b981" name="Energy Generated (kWh)" />
        ...
        <Card className="bg-gradient-to-r from-emerald-500/10 to-cyan-500/10 border-emerald-500/20">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              <p className="text-slate-400 text-sm mb-1">Total Generators</p>
              <p className="text-2xl font-bold text-white">{generators.length}</p>
            </div>
            <div>
              <p className="text-slate-400 text-sm mb-1">Tokens Generated</p>
              <p className="text-2xl font-bold text-emerald-400">{tokens.length}</p>
            </div>
            <div>
              <p className="text-slate-400 text-sm mb-1">Tokens Allocated</p>
              <p className="text-2xl font-bold text-cyan-400">
                {tokens.filter(t => t.status === 'allocated').length}
              </p>
            </div>
            <div>
              <p className="text-slate-400 text-sm mb-1">Certificates Issued</p>
              <p className="text-2xl font-bold text-blue-400">{certificates.length}</p>
            </div>
          </div>
        </Card>
      </div>
